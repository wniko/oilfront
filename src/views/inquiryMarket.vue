<template>
    <div>
        ...
        <!-- 复选框 -->
            ...
            <el-table-column prop="totalPurchaseNum" header-align="center" align="center" min-width="150" label="总采购量" />
            ...
            <el-table-column prop="deliveryTime" :formatter="r => formatDate(r.loadTime)" header-align="center" align="center" min-width="130" label="交货时间" />
            <el-table-column prop="deliveryWay" header-align="center" align="center" min-width="150" label="提货方式" />
            ...
            <el-table-column prop="city" header-align="center" align="center" min-width="150" label="区域" />
        ...
        <inquiry-market-account v-if="addOrUpdateVisible" ref="inquiryMarketAccount" @refreshDataList="loadDataList"></inquiry-market-account>
        <inquiry-market-add-or-update v-if="addOrUpdateVisible" ref="inquiryMarketAddOrUpdate" @refreshDataList="loadDataList"></inquiry-market-add-or-update>
        <inquiry-market-play-video v-if="addOrUpdateVisible" ref="inquiryMarketPlayVideo" @refreshDataList="loadDataList"/>
    </div>
</template>

<script>
//引入弹窗页面
import InquiryMarketAccount from './inquiryMarket-account.vue';
import InquiryMarketAddOrUpdate from './inquiryMarket-add-or-update.vue';
import InquiryMarketPlayVideo from './inquiryMarket-play-video.vue';
export default {
    components: {
        InquiryMarketAccount,
        InquiryMarketAddOrUpdate,
        InquiryMarketPlayVideo
    },
    //模型层，定义变量
    data() {
        return {
    ...
    methods: {
        loadDataList: function() {
            ...
            //提交ajax请求
            that.$http('inquiryMarket/listByPage', 'POST', data, true, function(resp) {
        ...
        addHandle: function() {
            ...
            this.$nextTick(() => {
                this.$refs.inquiryMarketAddOrUpdate.init();
            });
        },
        //修改的操作
        updateHandle: function(id) {
            this.addOrUpdateVisible = true;
            this.$nextTick(() => {
                this.$refs.inquiryMarketAddOrUpdate.init(id);
            });
        },
        playing: function(val){
            this.addOrUpdateVisible = true;
            this.$nextTick(() => {
                this.$refs.inquiryMarketPlayVideo.init(val);
            });
        },
        //查看的操作
        findAccount: function(value) {
            this.addOrUpdateVisible = true;
            this.$nextTick(() => {
                this.$refs.inquiryMarketAccount.init(value);
            });
        },
        ...
        review: function(id){
            ...
            if (ids.length == 0) {
                ...
                that.$confirm('确定要审核选中的记录？', '提示', {
                    ...
                    that.$httpGet('inquiryMarket/review', 'GET', { inquiryMarketId: ids }, true, function(resp) {
                        if (resp.success) {
                            that.$message({
                                message: '操作成功',
                                type: 'success',
        ...
        finish: function(id){
            ...
            if (ids.length == 0) {
                ...
                that.$confirm('确定要结束选中的记录？', '提示', {
                    ...
                    that.$httpGet('inquiryMarket/finish', 'GET', { inquiryMarketId: ids }, true, function(resp) {
                        if (resp.success) {
                            that.$message({
                                message: '操作成功',
                                type: 'success',
        ...
        offline: function(id){
            ...
            if (ids.length == 0) {
                ...
                that.$confirm('确定要下线选中的记录？', '提示', {
                    ...
                    that.$httpGet('inquiryMarket/offline', 'GET', { inquiryMarketId: ids }, true, function(resp) {
                        if (resp.success) {
                            that.$message({
                                message: '操作成功',
                                type: 'success',
